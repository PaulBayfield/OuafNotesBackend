...
from .objects import Profil, Matiere, Note, Details, MCC, Competence, MatiereCoefficient, Coefficient, Absence
from .exceptions import InvalidCredentials, UnknownError, NotConnected
...
class Client(CAS):
    def __init__(self, service: str, session: ClientSession):
    ...
    async def profil(self) -> Profil:
        ...
        if res:
            if not self.__profil:
                ...
                self.__profil = Profil(re.compile(r"\/fr\/etudiant\/profil\/(.*\..*)\/.*").match(str(b.find("a", {"class": "nav-link changeprofil"})).split("href=\"")[1].split("\"")[0])[1])
    ...
    async def notes(self) -> list[Matiere]:
        ...
        url = urljoin(self.BASE_URL, f"/fr/etudiant/profil/{self.__profil.raw}/notes")
    ...
    async def absences(self) -> list[Absence]:
        ...
        url = urljoin(self.BASE_URL, f"/fr/etudiant/profil/{self.__profil.raw}/absences")
